...
function syntaxHighlightCsp(str) {
    ...
    try {
        ...
        if (validate) {
            ...
            for (const { name, value } of originalPolicies) {
                console.log(syntaxHighlightCsp(reformatCsp(name, value)));
            }
            ...
            if (policiesWithAdditions.length > 0) {
                ...
                for (const policy of policiesWithAdditions) {
                    for (const directive of Object.keys(policy.additions)) {
                        console.error(syntaxHighlightCsp(`  ${kebabCase(directive)} ${Object.keys(policy.additions[directive]).join(' ')}`));
            ...
            if (contentSecurityPolicy || !contentSecurityPolicyReportOnly) {
                console.log(syntaxHighlightCsp(reformatCsp('Content-Security-Policy', contentSecurityPolicy)));
            }
            if (contentSecurityPolicyReportOnly) {
                console.log(syntaxHighlightCsp(reformatCsp('Content-Security-Policy-Report-Only', contentSecurityPolicyReportOnly)));
            }
        }
        if (errors.length > 0) {
            process.exit(1);
