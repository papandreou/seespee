...
const {
    ...
    .option('user-agent', {
        type: 'string',
        description: 'Use a specific User-Agent string when retrieving http(s) resources. Useful with servers that are configured to only send a Content-Security-Policy header to browsers known to understand it'
    })
    .demand(1)
    .argv;
...
(async () => {
    try {
        const {
            ...
            policies,
            originalPolicies
        } = await seespee(nonSwitchArguments[0], { include, ignoreExisting, root, level, userAgent });
